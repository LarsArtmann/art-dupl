...
func singleMethod() string {
	...
	Context("When configuration file has filtering settings", func() {
		It("should load filter-generated setting from config", func() {
			...
			code := `package main
func filterGen() {}`
			runConfigTest(configContent, code, []string{"filter1.go", "filter2.go"})
		})
...
package db
func SQLCQuery() {}`
			runConfigTest(configContent, code, []string{"query1.go", "query2.go"})
		})
...
func TemplComponent() templ.Component {
	return nil
			...
			runConfigTest(configContent, code, []string{"comp1.go", "comp2.go"})
		})
	})
