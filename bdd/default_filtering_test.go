...
func assertSQLCFileFiltered(setup *testutil.BDDTestSetup, filename, functionName string) {
	regularCode := "package main\n" + testutil.DuplicateFuncSource(functionName)
	generatedCode := "package db\n" + testutil.DuplicateFuncSource(functionName)
...
func assertGeneratedFileIncluded(
	...
	args := append([]string{"-t", threshold}, extraArgs...)
...
func query() {
	for i := 0; i < 10; i++ {
		...
		It("should include SQLC files when --include-generated sqlc is used", func() {
...
` + testutil.DuplicateFuncSource("process")
			...
			output, err := setup.RunArtDupl("--include-generated", "templ,sqlc", "--threshold", "1")
			Expect(err).ToNot(HaveOccurred())
