...
func pkgFunc() {}`
	...
	func testWithFilterFlag(testCode string, filenames []string, flag string) {
		testWithStats(testCode, filenames, flag)
	}
...
func artDuplTest() string {
	...
	Context("When running stats with filter options", func() {
		It("should work with --filter-generated flag", func() {
			code := `package main
func filterTest() {}`
			testWithFilterFlag(code, []string{"filter1.go", "filter2.go"}, "--filter-generated")
		})
...
package db
func SQLCFunc() {}`
			testWithFilterFlag(code, []string{"sqlc1.go", "sqlc2.go"}, "--include-sqlc")
		})
	})
