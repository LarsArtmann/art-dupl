...
func pkgFunc() {}`
			...
			output, err := setup.RunSubcommand("stats", "--threshold", "1", specificPath)
			Expect(err).ToNot(HaveOccurred())
...
func artDuplTest() string {
	...
	Context("When running stats with filter options", func() {
		DescribeTable(
			...
			func(testCode string, filenames []string, flags ...string) {
				testWithFilterFlag(setup, testCode, filenames, flags...)
			},
			Entry(
...
func testWithFilterFlag(
	...
	testWithStats(setup, testCode, filenames, flags...)
}
