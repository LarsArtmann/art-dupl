...
| **BuildFlow Provider** | FULLY_FUNCTIONAL | `pkg/provider` self-registers art-dupl as a BuildFlow tool via the go-finding toolsdk (`toolsdk.Register` at init). BuildFlow consumes it with one blank import — no executor glue. `Detect` runs the public SDK (semantic mode, default threshold) on the working dir from context and returns one finding per clone occurrence with GroupID, severity ladder, snippets, and sibling links. The crawl honors `.gitignore` (shared `internal/gitignore` matcher, B1), and severity is capped at warning (detector-only, gate-safe). Classification metadata is CLI-only by decision (ADR-0025); `internal/jsonv2gate` guards the JSON invariant; verified LIVE in BuildFlow 2026-09-25 (E2E, cache, budget, lane split). |
