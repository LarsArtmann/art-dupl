...
const (
	...
	// conversion needed for mockgen/stringer detection.
	generatedHeader = "Code generated"
)

// markerBytes are the []byte forms of the generation markers, allocated once
// at package init so repeated bytes.Contains checks do not allocate per file.
var (
	generatedHeaderBytes = []byte(generatedHeader)
	sqlcMarkerBytes      = []byte(sqlcMarker)
	templMarkerBytes     = []byte(templMarker)
	protobufMarkerBytes  = []byte(protobufMarker)
)
...
func matchedGeneratedCategory(content []byte) (gogenfilter.FilterReason, bool) {
	if !bytes.Contains(content, generatedHeaderBytes) {
		return "", false
	}

	switch {
	case bytes.Contains(content, templMarkerBytes):
		return gogenfilter.ReasonTempl, true
	case bytes.Contains(content, sqlcMarkerBytes):
		return gogenfilter.ReasonSQLC, true
	case bytes.Contains(content, protobufMarkerBytes):
