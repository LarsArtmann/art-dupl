...
- **OPEN: destructive serialization + Type-2 classification** (2026-06-30 review): `syntax.Serialize`→`serial()` overwrites `n.Type` in place (`fingerprintSubtree`), so it is destructive on shared trees; calling it twice double-hashes. `printer/clone_processor.go::classifyCloneType` calls `syntax.Serialize` on nodes the suffix tree already serialized, and the walk short-circuits on `Statement=true` nodes — so the Type-1-vs-Type-2 (renamed) distinction is silently lost for statement-rooted fragments. Fix: serialize into a side table, never mutate `n.Type`; walk `node.Children` directly for Name comparison. See `docs/reviews/2026-06-30_18-31_full-code-review.html`.
- **OPEN: incremental cache-miss aliasing**: `job/incremental.go` cache-miss path returns the same node slice it stores in the cache (the hit path correctly `Clone()`s); `parseFile` is not concurrent-safe. Needs deep-clone-on-store + `singleflight`.
