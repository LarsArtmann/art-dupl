...
- **Lint config**: `_test.go` files are excluded from `exhaustruct` and `gochecknoglobals`. `cobra.Command` is in the `exhaustruct` exclude list (30+ fields, intentional partial initialization). `recvcheck` is enabled but not excluded anywhere.
