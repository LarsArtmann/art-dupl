...
- **Config validation**: `config.ValidateConfig` now validates `SortBy`, `DiffMode`, `TestThreshold` (enums via `IsValid()`, non-negative). When adding a typed enum/validated field to `Config`, add a validator to the chain in `config/config_validate.go`. **Config has custom `UnmarshalJSON`** (`config_migrate.go`) that converts legacy `"semantic": false` → `"detectionMode": "exact"`. Uses type-alias trick to prevent recursion.
