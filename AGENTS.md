...
- **Lint config**: `_test.go` and `bdd/` files are excluded from `gochecknoglobals`, `mnd`, `varnamelen`, `goconst`, and `exhaustruct`. `exhaustruct` and `tagliatelle` are DISABLED globally (exhaustruct: impractical for Go zero-value initialization; tagliatelle: codebase has mixed camelCase/snake_case JSON conventions). `varnamelen` has a comprehensive `ignore-names` list of idiomatic Go short names. `mnd` has common numbers in `ignored-numbers`. `recvcheck` is enabled but not excluded anywhere.
