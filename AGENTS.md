...
- **Idiom category**: Clones with <5 tokens AND ≤5 lines are classified as `idiom` (domain.CategoryIdiom) instead of their AST category. These are structural artifacts with near-zero actionability. The line guard prevents coarse-grained AST nodes (e.g. a templ Element covering many lines of attributes) from being dismissed as idioms.
