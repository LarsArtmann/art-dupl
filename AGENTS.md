...
**os.Exit removal & path validation (2026-05-23):** Removed `statError()` (called `os.Exit(1)`) from `cmd/run_crawl.go`. Crawl functions now log to stderr and skip invalid paths. Added `validatePaths()` in `cmd/run_analysis.go` that errors only when ALL specified paths are invalid (none exist). Fixes: cmd and BDD test suites now pass reliably (previously `os.Exit(1)` killed the test process). Also extracted `startProfiling`/`endProfiling` helpers from `executeAnalysis` to keep it under 80-line funlen limit.

**BDD test infrastructure fixes (2026-05-23):** Fixed `prepareSubcommandArgs` in `internal/testutil/bdd_runners.go` — it was treating flag values (e.g., `"5"` for `--threshold`) as paths, preventing `TmpDir` from being appended. Now tracks flag-value pairs using a `valueFlags` map so numeric/string flag values aren't mistaken for paths. Updated 4 BDD test assertions to match corrected error handling behavior (empty stdin → error; invalid paths → error; non-existent paths via RunArtDupl → graceful skip).
