...
- [ ] **go/types integration (`--type-aware` mode)** — The highest-impact false-positive reduction. art-dupl is purely syntax-based; it cannot distinguish `a.String()` (time.Time) from `b.String()` (*big.Int). Requires `golang.org/x/tools/go/packages` (10-100x slower than parsing). Opt-in flag. ~4h+ prototype.
