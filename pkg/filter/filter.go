...
func matchPattern(path, pattern string) bool {
	...
	// If pattern contains a path separator, match against full path or path suffix
	if strings.Contains(pattern, string(filepath.Separator)) || strings.Contains(pattern, "/") {
		// Try matching the full path
		fullPattern := "^" + regexPattern + "$"
		matched, err := regexp.MatchString(fullPattern, path)
		if err == nil && matched {
			return true
		}

		// Try matching just the path suffix (allows matching "vendor/*" against "/tmp/xxx/vendor/file.go")
		suffixPattern := regexPattern + "$"
		matched, err = regexp.MatchString(suffixPattern, path)
		if err == nil && matched {
			return true
		}
	...
	// Pattern has no path separator, match against just the filename
	filename := filepath.Base(path)
	filenamePattern := "^" + regexPattern + "$"
	matched, err := regexp.MatchString(filenamePattern, filename)
	if err != nil {
		return false
	}
	return matched
...
func isSQLCGenerated(filePath string, content string) bool {
	filename := filepath.Base(filePath)
...
func isTemplGenerated(filePath string, content string) bool {
	filename := filepath.Base(filePath)
