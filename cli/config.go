...
func NewCLIConfig() *CLIConfig {
	return &CLIConfig{
		...
		ThresholdLong: flag.Int(
			"threshold",
			15,
			"minimum token sequence size to consider as clone",
		),
		...
		SimpleJSON: flag.Bool(
			"simple-json",
			false,
			"output simple JSON format (legacy format from duplicates project)",
		),
		Plumbing: flag.Bool(
			"plumbing",
			false,
