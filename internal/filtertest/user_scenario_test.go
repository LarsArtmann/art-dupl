...
func writeConfigFile(t *testing.T, path, filename, content string) {
	t.Helper()
...
func writeSQLCFile(t *testing.T, path, filename, content string) {
	t.Helper()
	if err := os.WriteFile(path, []byte(content), 0o644); err != nil {
		t.Fatalf("failed to write %s: %v", filename, err)
	}
...
func writeGoFile(t *testing.T, path, filename, content string) {
	t.Helper()
	if err := os.WriteFile(path, []byte(content), 0o644); err != nil {
		t.Fatalf("failed to write %s: %v", filename, err)
	}
...
`
	...
	writeConfigFile(t, configPath, "sqlc.yaml", sqlcConfig)
...
`
	writeSQLCFile(t, filepath.Join(queriesDir, "articles.sql.go"), "articles.sql.go", articlesSQLC)
...
`
	writeSQLCFile(t, filepath.Join(queriesDir, "users.sql.go"), "users.sql.go", usersSQLC)
...
`
	writeGoFile(t, filepath.Join(repoDir, "article_repository.go"), "article_repository.go", articleRepo)
...
`
	writeGoFile(t, filepath.Join(repoDir, "mock_repository.go"), "mock_repository.go", mockRepo)
...
`
	...
	writeSQLCFile(t, sqlcFilePath, "users.sql.go", sqlcFile)
