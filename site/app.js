...
(function () {
    ...
    for (let i = 0; i < word.length; i++) {
      ...
      if (!node.children[ch]) {
        node.children[ch] = {
          ...
          count: 0,
          char: ch,
          depth: node.depth !== undefined ? node.depth + 1 : 0,
        };
      }
  ...
  function flattenTree(node, x, y, spread, parent) {
    ...
    nodeList.push({
      x,
      y,
      r: Math.min(2 + (node.count || 1) * 0.3, 5),
      alpha: 0.2 + Math.min(node.count || 0, 10) * 0.06,
      char: node.char || "",
      depth: node.depth || 0,
    });
    ...
    if (parent) {
      edgeList.push({
        ...
        x2: x,
        y2: y,
        alpha: 0.08 + Math.min(node.count || 0, 8) * 0.01,
      });
    }
