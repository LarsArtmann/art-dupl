...
**All 6 FULLY DONE items shipped.** Detected-vs-actionable summary (`dbed8cbe`), 4 new actionability patterns (`3b202e8c`, `1b6f7441`), baseline/directive split-brain fix (`7b28ac0d`, `3ca3d643`), category expansion (`3ca3d643`), --min-tokens (`fd3b0d38`), suggest-generics help text (`caa14ae9`). Section c (15 NOT STARTED items): //go:embed, go-http-error-guard, sync-rwmutex-read, etc. → TODO_LIST (feedback-driven patterns). Section f (49 brainstorm items): routed to TODO_LIST (bounded) and ROADMAP (long-term). HTML output improvements → TODO_LIST.
