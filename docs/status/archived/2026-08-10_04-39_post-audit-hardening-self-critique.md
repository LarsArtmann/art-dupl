...
**All 6 hardening tasks shipped.** Cache error-path tests, concurrent prune race test, KeyWithParams collision fix, EraseHash invariant validation, gopls documentation, and benchmark baseline count=5 are all done. Section c/f items (CacheVersion bump, in-memory LRU, hysteresis pruning, TypeAwareData restructure) → TODO_LIST MEDIUM/DEFERRED. Questions Q1-Q3: Q1 (CacheVersion bump) → TODO_LIST; Q2 (TypeAwareData restructure) → TODO_LIST DEFERRED; Q3 (in-memory LRU) → TODO_LIST.
