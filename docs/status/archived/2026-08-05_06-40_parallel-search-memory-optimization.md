...
**Core work shipped.** Parallel suffix tree search (`--search-workers`) and memory-compact `[]TokenValue` storage are in CHANGELOG `[Unreleased]` → Added. See ADR-0019. Section B/C items (streaming, HOW_TO_USE update, fuzz tests) — HOW_TO_USE done, fuzz tests shipped, streaming → ROADMAP.
