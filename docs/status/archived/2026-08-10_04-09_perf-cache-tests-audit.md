...
**Superseded by 2026-08-10_04-39 (post-audit hardening).** Cache key isolation bug fixed (`956c1125`), Prune test coverage fixed, crashing benchmarks fixed, benchmark baseline committed. The NOT STARTED items (cache error-path tests, concurrent prune test, gopls warnings) were all completed in the follow-up session (`70afabe4`, `8a266a5d`, AGENTS.md doc). CacheVersion bump and in-memory LRU → TODO_LIST.
