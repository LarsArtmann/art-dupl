...
**Core work shipped.** Bool-accumulator-initializer pattern is FULLY_FUNCTIONAL with all three Go forms (`:=`, `var`, package-level). Section c items (idiom category, := encoding, broader dogfooding) are aspirational → ROADMAP. Tagliatelle fix → TODO_LIST.
