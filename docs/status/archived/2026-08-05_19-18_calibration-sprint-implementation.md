...
**All 4 HIGH items shipped.** interface-assertion, type-alias-block, test-helper-delegate, demo-dir exclusion, --show-suppressed all in CHANGELOG `[Unreleased]` → Added. Section b/c items (SDK wiring, re-calibration) → ROADMAP/TODO_LIST.
