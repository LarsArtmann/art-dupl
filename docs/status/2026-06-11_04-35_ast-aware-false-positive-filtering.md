...
- `EvaluateActionabilityWithLabel()` returns `(PatternLabel, CloneActionability)` — identifies *which* pattern was detected
...
- Non-actionable clones get `PriorityLow` and specific suggestions explaining *why* they're not actionable
...
- **`isTestScaffolding`** currently detects assertion method names via a hardcoded list (`Expect`, `NotTo`, `Equal`, `HaveLen`, etc.). This covers Ginkgo and testify but could miss custom assertion libraries or DSL-like test frameworks. The pattern is *structurally correct* but the name list is intentionally conservative.
