...
Worse, `--semantic` mode is **backwards**: it bakes exact identifier names into the token hash (`identifier_hash.go:52`), making matches *stricter* than structural mode. `processUser` and `processOrder` with identical logic are *rejected* because the names differ. That's exact-text matching, not semantic matching.
