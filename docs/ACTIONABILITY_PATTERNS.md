...
| Interface method      | `interface-method`             | FuncDecl body matching an interface contract with ≤4 body statements. Two paths: type-aware (go/types checks same-package interfaces) and static fallback (stdlib name list)  | `func (t Time) String() string { ... }`     |
...
- **Interface method detection**: The `interface-method` pattern has two complementary detection paths. When `--type-aware` is active, the transformer calls `golang.IsInterfaceMethod()` which uses `go/types` to check if a FuncDecl satisfies a same-package interface (including embedded interfaces, value and pointer receivers). The `InterfaceMethod` flag on `domain.CloneNode` carries this to the actionability layer. As a static fallback, the `commonInterfaceMethodNames` list covers 25+ stdlib interface method names (fmt.Stringer, io.Reader, error, etc.) that are not declared in the same package. Body size limit is ≤4 statements. Cross-package interface scanning remains future work (see ROADMAP).
