...
**ADDRESSED (3/4 groups fixed).** `defer-call` pattern catches `defer unsubscribe()`. `empty-default` pattern catches `if x == "" { x = default }`. `state-flag-mutation` catches `x.flag = true`. Remaining false positive is in TODO_LIST for investigation.
