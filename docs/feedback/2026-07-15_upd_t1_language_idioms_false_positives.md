...
| `return false`                         | `config.go`, `npm.go`, `engine.go`, `manifest.go`           | 4+    | Standard boolean guard result.                 |
...
| `err := json.Unmarshal(raw, &v)`       | `npm.go`, `packagejson.go`                                  | 3     | Standard JSON parsing pattern; targets differ. |
| `_, err := dec.ReadToken()`            | `packagejson.go`                                            | 2     | JSON decoder streaming pattern.                |
| `seconds, err := strconv.Atoi(header)` | `npm.go`, `progress.go`                                     | 2     | Standard string parsing.                       |
